import { useContext, useState } from 'react';
import { StyleSheet, View, Text, TouchableOpacity, Platform, SafeAreaView } from 'react-native';
import Feather from '@expo/vector-icons/Feather';
import Ionicons from '@expo/vector-icons/Ionicons';
import { AuthContext } from '@contexts/AuthContext';
import { MaterialCommunityIcons } from '@expo/vector-icons';
import { useNavigation } from '@react-navigation/native';

interface HeaderProps {
    MarginTopCustom?: number;
    title: string;
    iconBef?: React.ComponentProps<typeof Ionicons>['name'];
    pressBef?: () => Promise<void>;
    iconAft?: React.ComponentProps<typeof Feather>['name'];
    pressAft?: () => void;
}
export default function Header({ title, iconBef, pressBef, iconAft, pressAft }: HeaderProps) {
    const { theme } = useContext(AuthContext);
    const navigation = useNavigation<any>();
    return (
        <SafeAreaView>
            {
                Platform.OS == 'web'
                    ?
                    < View style={[style.container, { justifyContent: 'space-between' }]} >
                        <View style={{ flexDirection: 'row' }}>
                             {iconBef && pressBef ?
                            <TouchableOpacity
                                onPress={pressBef}
                            >
                                <Ionicons name={iconBef} size={35} color={theme ? '#f6b10a' : '#2f1b36'} />
                            </TouchableOpacity>
                            : null
                        }
                            <Text style={[style.title, { color: '#f6b10a' }]}>{title}</Text>
                        </View>

                        <View style={{ flexDirection: 'row' }}>
                            <TouchableOpacity
                                style={{ margin: 15 }}
                                onPress={() => navigation.navigate('StackHome')}>
                                <Feather name="home" size={24} color="#f6b10a" />
                            </TouchableOpacity>
                              <TouchableOpacity
                                style={{ margin: 15 }}
                                onPress={() => navigation.navigate('StackDiscover', {
                                    screen: 'Discover'
                                })}
                            >
                                <Feather name="compass" size={24} color="#f6b10a" />
                            </TouchableOpacity>
                            <TouchableOpacity
                                style={{ margin: 15 }}
                                onPress={() => navigation.navigate('StackDiscover', {
                                    screen: 'Search'
                                })}
                            >
                                <Feather name="search" size={24} color="#f6b10a" />
                            </TouchableOpacity>
                            <TouchableOpacity
                                style={{ margin: 15 }}
                                onPress={() => navigation.navigate('StackAccount')}
                            >
                                <MaterialCommunityIcons name="account" size={24} color="#f6b10a" />
                            </TouchableOpacity>
                             <TouchableOpacity
                                style={{ margin: 15 }}
                                onPress={() => navigation.navigate('Info')}
                            >
                                <MaterialCommunityIcons name="information" size={24} color="#f6b10a" />
                            </TouchableOpacity>
                        </View>

                    </View >
                    :
                    < View style={style.container} >
                        {iconBef && pressBef ?
                            <TouchableOpacity
                                onPress={pressBef}
                            >
                                <Ionicons name={iconBef} size={35} color={theme ? '#f6b10a' : '#2f1b36'} />
                            </TouchableOpacity>
                            : null
                        }
                        <Text style={[style.title, { color: '#f6b10a' }]}>{title}</Text>
                        {
                            iconAft && pressAft ?
                                <TouchableOpacity
                                    style={style.button}
                                    onPress={pressAft}
                                >
                                    <Feather name={iconAft} size={30} color={theme ? '#f6b10a' : '#2f1b36'} />
                                </TouchableOpacity>
                                : null
                        }

                    </View >
            }

        </SafeAreaView>
    );
}

const style = StyleSheet.create({
    container: {
        marginTop: 25,
        marginHorizontal: 25,
        height: 55,
        alignItems: 'center',
        flexDirection: 'row'
    },
    title: {
        marginHorizontal: 15,
        fontSize: 30,
        fontWeight: 'bold',
    },
    button: {
        position: 'absolute',
        right: 10,
    }
})